...
class Split:
    ...
    def generators(self,
        ...
        self.train_gen = SplitGenerator(
            data_df=self.train_purchases_txt(),# 구매한 유저 아이디 데이터
            itemizer=self.master_data.toki, # 토크나이저 생성
...
class SplitGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        if self.full_data:
            ret_x.append(self.toki.texts_to_matrix(data_slice[:, 1], mode=mod)) # 각 data 의 item token화
